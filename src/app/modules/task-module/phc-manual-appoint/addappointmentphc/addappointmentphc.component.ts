...
@Component({
  selector: 'app-addappointmentphc',
  templateUrl: './addappointmentphc.component.html',
  styleUrls: ['./addappointmentphc.component.scss']
})
export class AddappointmentphcComponent implements OnInit {
  ...
  constructor(private cdr: ChangeDetectorRef, private dialogRef: MatDialogRef<AddappointmentphcComponent>, public dialog: MatDialog, @Inject(MAT_DIALOG_DATA) private _data: any,
  ...
  fixappointment = new FormGroup({
    ...
    select_toTime: new FormControl('', [Validators.required, this.timeComparisonValidator]),
    appt_DO_Id_FK: new FormControl(''),
  ...
  ngOnInit(): void {
    ...
    // this.bind();
    this.apid = this._data.pR_Id
    //john
    this.fixappointment.value.appt_PatientId_FK = this.apid;
  ...
  timeComparisonValidator(control: any): { [key: string]: boolean } | null {
    const fromTime = control.parent?.get('select_FrmTime').value;
  ...
  bind() {
    ...
    this.PR_Id = this._data.pR_Id
    this.patientName = this._data.pR_Name
    this.PR_PatientCode = this._data.pR_PatientCode
    this.Dob = this._data.pR_DOB.slice(0, 10)
    this.Age = this._data.pR_Age
    this.Gender = this._data.pR_Gender
    this.Bg = this._data.pR_BloodGroup
    this.Image_Url = environment.Image_Url + 'Images/' + this._data.pR_Photo
    console.log('patient photo :' + this.Image_Url)
  }
  ...
  // select_totime: any;
  // Get_Doctor(select_day: any, select_frmtime: any, event: any) {
  //   if (select_day == null || select_day == '') {
  //     this.notif.errorsmsg('Please enter date')
  //     return;
  //   }
  //   if (select_frmtime == null || select_frmtime == '') {
  ...
  //   var datePipe = new DatePipe('en-US');
  //   select_day = datePipe.transform(select_day, 'dd/MM/yyyy');
  //   console.log("selectday", select_day)
  //   this.service.Get_Doctor(select_day, select_frmtime, this.select_totime)
  //     .subscribe((data) => {
  //       this.doctordd = data
  ...
  openDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(`Dialog result: ${result}`);
      if (result == true) {
        this.router.navigate(['/base/task/appointment/previous-record/' + this.PR_Id]);
      }
      else {
        // console.log("No Previous Record");
        this.router.navigate(['/base/task/manualappointmentphc']);
  ...
  updateform() {
    ...
    this.fixappointment.value.appt_PatientId_FK = this.PR_Id;  //this.patient.value.PR_Id;
